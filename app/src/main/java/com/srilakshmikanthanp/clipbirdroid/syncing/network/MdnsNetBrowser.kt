...
class MdnsNetBrowser(private val context: Context) : NetBrowser {
  private val nsdManager: NsdManager = context.getSystemService(Context.NSD_SERVICE) as NsdManager
  private val wifiManager = context.getSystemService(Context.WIFI_SERVICE) as WifiManager
  private val listeners: MutableList<NetBrowserListener> = mutableListOf()
  ...
  override fun removeListener(listener: NetBrowserListener) {
    listeners.remove(listener)
  }

  override fun addListener(listener: NetBrowserListener) {
    listeners.add(listener)
  }

  private fun notifyServiceRemoved(device: NetResolvedDevice) {
    for (l in listeners) l.onServiceRemoved(device)
  }

  private fun notifyServiceAdded(device: NetResolvedDevice) {
  ...
  private fun serviceResolved(info: NsdServiceInfo) {
    ...
    // notify the listeners
    notifyServiceAdded(NetResolvedDevice(serviceName, host, port))
  }
  ...
  private fun serviceLost(info: NsdServiceInfo) {
    ...
    // notify the listeners
    notifyServiceRemoved(NetResolvedDevice(serviceName, ip, port))
  }
  ...
  override fun start() {
    if (listener.get() != null) throw IllegalStateException("Browser is already started")
