...
@Composable
private fun LargeImagePreview(image: Int) {
  ...
  Card(modifier = modifier) {
    Column(modifier = Modifier.padding(5.dp)) {
      ...
      Row(
        ...
        for (clip in content) {
          if (clip.mimeType == Clipboard.MIME_TYPE_PNG && clip.data.size < maxImgSize) {
            Image(clip.data.toBitmap(), stringResource(id = R.string.clipbird_content))
            break
          } else if (clip.mimeType == Clipboard.MIME_TYPE_PNG) {
            LargeImagePreview(R.drawable.photo)
            break
          }

          if (clip.mimeType == Clipboard.MIME_TYPE_TEXT) {
            val text = String(clip.data).take(maxTxtSize)
...
@Composable
private fun ClipboardContentTilePreview() {
  val content = listOf(ClipboardContent(Clipboard.MIME_TYPE_TEXT, "This is a sample clipboard text item.".toByteArray()))
