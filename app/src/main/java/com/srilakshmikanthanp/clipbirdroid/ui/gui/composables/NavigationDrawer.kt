...
enum class DrawerItems {
  DEVICES, HISTORY, TRUSTED_DEVICES, SETTING, ABOUT
}
...
private fun NavigationDrawerContent(
  ...
  ModalDrawerSheet (
    ...
    Column (modifier = Modifier.padding(10.dp)) {
      ...
      NavigationDrawerItem(
        onClick =  { onItemClicked(DrawerItems.TRUSTED_DEVICES) },
        label = { Text(text = stringResource(id = R.string.trusted_devices)) },
        selected = selected == DrawerItems.TRUSTED_DEVICES,
      )

      NavigationDrawerItem(
        onClick =  { onItemClicked(DrawerItems.SETTING) },
        label = { Text(text = stringResource(id = R.string.settings)) },
        selected = selected == DrawerItems.SETTING,
      )
...
fun NavigationDrawer(
  ...
  ModalNavigationDrawer(
    drawerContent = { NavigationDrawerContent(onItemClicked, selected) },
    drawerState = drawerState
  ) {
    content()
  }
